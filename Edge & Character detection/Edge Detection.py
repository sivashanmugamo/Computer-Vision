"""
The aim is to experiment with two commonly used edge detection operator, i.e., Prewitt operator and Sobel operator,
and to get familiar with 'tricks', e.g., padding, commonly used by computer vision 'researchers'.
...
def normalize(img):
    ...
    img_min = min([min(y) for y in img])
...
def edge_magnitude(edge_x, edge_y):
    ...
    edge_x2 = utils.elementwise_mul(edge_x, edge_x)
    edge_y2 = utils.elementwise_mul(edge_y, edge_y)

    edge_sum = utils.elementwise_add(edge_x2, edge_y2)

    edge_mag = np.sqrt(edge_sum)
